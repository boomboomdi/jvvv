...
//          echo "$key - $value \n";
    ...
    {
        return $this->redisObj[$this->sn]->lTrim($key, $start, $stop);
    }

    public function lrange($key, $start, $end)
    {
        return $this->redisObj[$this->sn]->lRange($key, $start, $end);
    }

    public function blpop($key, $timeout)
    {
        return $this->redisObj[$this->sn]->blPop($key, $timeout);
    }

    public function brpop($key, $timeout)
    {
        return $this->redisObj[$this->sn]->brPop($key, $timeout);
    }
    /*------------------------------------2.end list结构----------------------------------------------------*/
